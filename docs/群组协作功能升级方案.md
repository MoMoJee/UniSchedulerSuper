# 群组协作功能升级方案

**文档版本**: v1.0  
**创建日期**: 2025-11-11  
**状态**: 开发中

---

## 📋 待办事项清单 (TODO List)

### Phase 1: 数据模型设计 ✅
- [x] 1.1 数据库模型设计
- [x] 1.2 UserData 扩展设计
- [x] 1.3 数据流转逻辑设计

### Phase 2: 后端实现 ⏳
- [ ] 2.1 数据库迁移脚本
- [ ] 2.2 models.py 修改
- [ ] 2.3 views_share_groups.py 核心逻辑
- [ ] 2.4 views_events.py 修改点
- [ ] 2.5 urls.py 路由配置
- [ ] 2.6 同步机制实现

### Phase 3: 前端实现 ⏳
- [ ] 3.1 选项卡切换 UI
- [ ] 3.2 编辑弹窗添加分享选项
- [ ] 3.3 群组管理界面
- [ ] 3.4 只读事件渲染
- [ ] 3.5 版本检测与自动刷新

### Phase 4: 测试与优化 ⏳
- [ ] 4.1 单元测试
- [ ] 4.2 集成测试
- [ ] 4.3 性能优化
- [ ] 4.4 边界条件处理

---

## 🎯 核心设计理念

### 设计原则
**"打标签" + "数据库查询"模式**

1. **分享机制**: 用户通过编辑日程，给 event 添加 `shared_to_groups: ["share_group_xxx"]` 标签
2. **数据汇总**: 群组数据库实时查询所有成员中带有该群组标签的日程
3. **数据获取**: 用户查看群组日程时，直接读取群组数据库（不缓存到本地）
4. **实时同步**: 编辑/删除操作后，立即触发群组数据库同步
5. **版本控制**: 通过版本号机制，让前端检测到数据更新并提示刷新

### 与现有系统的关系
- **日程组（groupID）**: 用户个人的日程分类，如"工作"、"学习"、"生活"
- **分享群组（share_group_id）**: 协作群组，多人共享日程
- **两者独立**: 同一个日程可以既属于某个日程组，又分享到某个群组

---

## 📊 Part 1: 数据模型设计

### 1.1 数据库模型

#### CollaborativeCalendarGroup（分享群组）
```python
class CollaborativeCalendarGroup(models.Model):
    """协作日历群组"""
    share_group_id = models.CharField(max_length=100, primary_key=True)
    share_group_name = models.CharField(max_length=200)
    share_group_color = models.CharField(max_length=20, default='#3498db')
    share_group_description = models.TextField(blank=True)
    owner = models.ForeignKey(User, on_delete=models.CASCADE, related_name='owned_share_groups')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    
    class Meta:
        db_table = 'collaborative_calendar_group'
        verbose_name = '协作日历群组'
        verbose_name_plural = '协作日历群组'
```

**字段说明**:
- `share_group_id`: 群组唯一标识，使用 `share_group_` 前缀避免与日程组 ID 冲突
- `share_group_name`: 群组名称，如"工作协作组"、"家庭日历"
- `share_group_color`: 群组颜色标签，用于前端显示
- `owner`: 群主，拥有管理权限
- `created_at/updated_at`: 时间戳

#### GroupMembership（群组成员关系）
```python
class GroupMembership(models.Model):
    """群组成员关系"""
    share_group = models.ForeignKey(
        CollaborativeCalendarGroup, 
        on_delete=models.CASCADE,
        related_name='memberships'
    )
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    role = models.CharField(max_length=20, default='member')  # owner/admin/member
    joined_at = models.DateTimeField(auto_now_add=True)
    
    class Meta:
        db_table = 'group_membership'
        unique_together = ('share_group', 'user')
        verbose_name = '群组成员'
        verbose_name_plural = '群组成员'
```

**字段说明**:
- `share_group`: 关联的群组
- `user`: 成员用户
- `role`: 角色
  - `owner`: 群主（创建者）
  - `admin`: 管理员（可邀请/移除成员）
  - `member`: 普通成员
- `unique_together`: 确保同一用户在同一群组中只有一条记录

#### GroupCalendarData（群组日历数据）
```python
class GroupCalendarData(models.Model):
    """群组日历数据 - 汇总所有成员的共享日程"""
    share_group = models.OneToOneField(
        CollaborativeCalendarGroup,
        on_delete=models.CASCADE,
        related_name='calendar_data'
    )
    events_data = models.JSONField(default=list)  # 所有共享事件列表
    last_updated = models.DateTimeField(auto_now=True)
    version = models.IntegerField(default=0)  # 版本号，用于前端检测更新
    
    class Meta:
        db_table = 'group_calendar_data'
        verbose_name = '群组日历数据'
        verbose_name_plural = '群组日历数据'
```

**字段说明**:
- `events_data`: JSON 格式，存储所有成员分享到该群组的日程
  ```json
  [
    {
      "id": "event_xxx",
      "title": "团队会议",
      "start": "2025-11-12T10:00:00",
      "end": "2025-11-12T11:00:00",
      "owner_id": 123,
      "owner_name": "张三",
      "is_readonly": true,
      "shared_at": "2025-11-11T09:00:00",
      ...其他 event 字段
    }
  ]
  ```
- `version`: 每次数据更新时递增，用于前端检测是否有新数据
- `last_updated`: 最后更新时间

---

### 1.2 UserData 扩展

#### events 字段扩展
在现有的 `DATA_SCHEMA["events"]["items"]` 中添加：

```python
"shared_to_groups": {
    "type": list,
    "nullable": False,
    "default": [],
    "description": "该日程分享到的群组列表"
}
```

**示例数据**:
```json
{
  "id": "event_123",
  "title": "项目讨论会",
  "groupID": "1",  // 日程组（个人分类）
  "shared_to_groups": ["share_group_work", "share_group_team"],  // 分享群组
  ...
}
```

**注意事项**:
- `groupID`: 用户个人的日程分类（现有功能，不变）
- `shared_to_groups`: 分享到的协作群组（新增功能）
- 两者互不干扰，可以同时存在

---

### 1.3 数据流转逻辑

#### 核心流程图
```
用户A编辑日程
    ↓
添加 shared_to_groups: ["share_group_work"]
    ↓
保存到 UserData["events"]
    ↓
触发同步任务 sync_group_calendar_data(["share_group_work"])
    ↓
查询 share_group_work 所有成员的 events
    ↓
筛选出 shared_to_groups 包含 "share_group_work" 的日程
    ↓
汇总到 GroupCalendarData.events_data
    ↓
递增 version 版本号
    ↓
用户B轮询检测到 version 变化
    ↓
重新加载群组日程
```

#### 关键点
1. **单向流动**: 用户日程 → 群组数据库（只读）
2. **实时同步**: 编辑后立即触发同步
3. **版本控制**: 通过 version 让前端知道何时刷新
4. **差异对比**: 记录"之前分享到哪些群组"，同步时处理增删改

---

## 🔄 Part 2: 核心业务流程

### 2.1 流程1：用户分享日程到群组

#### 场景描述
用户A有一个重复日程"每周例会"（FREQ=WEEKLY），想要分享到"工作协作组"

#### 前端操作流程
1. 用户点击编辑日程
2. 选择"编辑全部"模式（针对重复日程系列）
3. 在"分享设置"区域勾选"工作协作组"
4. 点击保存

**注意**: 除了 `bulk_edit_events_impl`，还有两个地方需要适配：
- **`update_events_impl`**: 用于拖拽事件改变时间，也支持字段更新，需要添加 `shared_to_groups` 处理
- **`create_event_impl`**: 创建日程时，需要支持同时分享到群组

#### 后端处理流程
```python
# views_events.py - bulk_edit_events_impl()

# 1. 接收前端数据
data = {
    "event_id": "event_123",
    "edit_scope": "all",  # 编辑整个系列
    "shared_to_groups": ["share_group_work"]  # 新增字段
}

# 2. 根据 edit_scope，找到该系列的所有事件
if edit_scope == 'all' and series_id:
    affected_events = [e for e in events if e['series_id'] == series_id]
else:
    affected_events = [target_event]

# 3. 记录"之前分享到哪些群组"（用于差异对比）
old_shared_groups = set()
for event in affected_events:
    old_shared_groups.update(event.get('shared_to_groups', []))

# 4. 更新所有事件的 shared_to_groups
new_shared_groups = set(data.get('shared_to_groups', []))
for event in affected_events:
    event['shared_to_groups'] = list(new_shared_groups)

# 5. 保存到 UserData
user_events_data.set_value(events)

# 6. 计算差异，触发同步
added_groups = new_shared_groups - old_shared_groups  # 新增的群组
removed_groups = old_shared_groups - new_shared_groups  # 移除的群组
all_affected_groups = added_groups | removed_groups

if all_affected_groups:
    sync_group_calendar_data(list(all_affected_groups), user)
```

#### 同步任务逻辑
```python
# views_share_groups.py - sync_group_calendar_data()

def sync_group_calendar_data(share_group_ids: list, trigger_user=None):
    """同步群组日历数据"""
    for group_id in share_group_ids:
        # 1. 获取群组所有成员
        memberships = GroupMembership.objects.filter(share_group_id=group_id)
        
        # 2. 汇总所有成员的共享日程
        all_shared_events = []
        
        for membership in memberships:
            # 获取成员的 events
            user_events_data = UserData.objects.get(
                user=membership.user, 
                key='events'
            )
            events = json.loads(user_events_data.value)
            
            # 筛选分享到该群组的日程
            shared_events = [
                e for e in events 
                if group_id in e.get('shared_to_groups', [])
            ]
            
            # 添加 owner 信息和只读标记
            for event in shared_events:
                event_copy = event.copy()
                event_copy['owner_id'] = membership.user.id
                event_copy['owner_name'] = membership.user.username
                event_copy['is_readonly'] = True
                event_copy['shared_at'] = datetime.now().isoformat()
                all_shared_events.append(event_copy)
        
        # 3. 保存到群组数据库，递增版本号
        group_data, created = GroupCalendarData.objects.get_or_create(
            share_group_id=group_id,
            defaults={'events_data': [], 'version': 0}
        )
        
        group_data.events_data = all_shared_events
        group_data.version += 1
        group_data.save()
        
        logger.info(f"Synced {len(all_shared_events)} events to group {group_id}, version {group_data.version}")
```

---

### 2.2 流程2：用户查看群组日程

#### 场景描述
用户B打开日历，点击"工作协作组"选项卡，查看群组内所有人的共享日程

#### 前端操作流程
1. 用户点击"工作协作组"选项卡
2. 前端发送 GET 请求，携带本地缓存的版本号
3. 后端返回群组日程数据（或告知无更新）
4. 前端渲染日程（只读模式）

#### API 设计
```python
# GET /api/share-groups/{share_group_id}/events/?version={local_version}

# 响应格式1：有更新
{
    "status": "updated",
    "version": 125,
    "events": [
        {
            "id": "event_123",
            "title": "项目讨论",
            "owner_id": 1,
            "owner_name": "张三",
            "is_readonly": true,
            ...
        }
    ],
    "last_updated": "2025-11-11T10:30:00"
}

# 响应格式2：无更新
{
    "status": "no_update",
    "version": 124
}
```

#### 后端实现逻辑
```python
# views_share_groups.py - get_share_group_events()

def get_share_group_events(request, share_group_id):
    """获取群组日程（带版本检测）"""
    local_version = int(request.GET.get('version', 0))
    
    # 1. 检查用户是否是该群组成员
    if not GroupMembership.objects.filter(
        share_group_id=share_group_id,
        user=request.user
    ).exists():
        return JsonResponse({'error': '您不是该群组成员'}, status=403)
    
    # 2. 获取群组数据
    try:
        group_data = GroupCalendarData.objects.get(share_group_id=share_group_id)
    except GroupCalendarData.DoesNotExist:
        return JsonResponse({
            'status': 'updated',
            'version': 0,
            'events': [],
            'last_updated': None
        })
    
    # 3. 版本检测
    if group_data.version == local_version:
        return JsonResponse({
            'status': 'no_update',
            'version': local_version
        })
    
    # 4. 返回最新数据
    return JsonResponse({
        'status': 'updated',
        'version': group_data.version,
        'events': group_data.events_data,
        'last_updated': group_data.last_updated.isoformat()
    })
```

---

### 2.3 流程3：用户编辑/删除已分享的日程

#### 场景描述
用户A编辑了已分享的日程"项目讨论"，修改了时间和标题

#### 关键逻辑
1. **利用现有编辑API**: 不需要新API，直接使用现有的编辑接口
   - `bulk_edit_events_impl` - 编辑弹窗保存时触发
   - `update_events_impl` - 拖拽事件或快速编辑时触发
2. **自动检测分享状态**: 后端检查 `shared_to_groups` 是否非空
3. **触发同步**: 如果有分享，自动触发 `sync_group_calendar_data`
4. **前端感知**: 用户B的前端轮询检测到版本变化，提示刷新

#### 代码修改点

##### 方案A：bulk_edit_events_impl（编辑弹窗）
```python
# views_events.py - bulk_edit_events_impl() 末尾

# 现有保存逻辑
user_events_data.set_value(updated_events)

# 新增：检测并同步群组数据
affected_groups = set()
for event in updated_events:
    old_groups = set(event.get('_old_shared_to_groups', []))
    new_groups = set(event.get('shared_to_groups', []))
    affected_groups.update(old_groups | new_groups)

if affected_groups:
    from .views_share_groups import sync_group_calendar_data
    sync_group_calendar_data(list(affected_groups), request.user)

return JsonResponse({'status': 'success'})
```

##### 方案B：update_events_impl（拖拽/快速编辑）
```python
# views_events.py - update_events_impl() 末尾

# 现有保存逻辑
user_events_data.set_value(events)

# 新增：检测并同步群组数据
affected_groups = set()

# 如果编辑单个实例
if rrule_change_scope == 'single':
    old_groups = set(target_event.get('_old_shared_to_groups', []))
    new_groups = set(target_event.get('shared_to_groups', []))
    affected_groups.update(old_groups | new_groups)

# 如果编辑整个系列
elif rrule_change_scope in ['all', 'future', 'from_time']:
    for event in events:
        if event.get('series_id') == series_id:
            old_groups = set(event.get('_old_shared_to_groups', []))
            new_groups = set(event.get('shared_to_groups', []))
            affected_groups.update(old_groups | new_groups)

if affected_groups:
    from .views_share_groups import sync_group_calendar_data
    sync_group_calendar_data(list(affected_groups), request.user)

return JsonResponse({'status': 'success'})
```

#### 删除日程的特殊处理
当用户删除已分享的日程时：
1. 从用户的 `events` 中移除
2. `sync_group_calendar_data` 会重新汇总，自动移除该日程
3. 群组数据库中的该日程消失
4. 其他成员刷新后看不到该日程

---

### 2.4 流程4：取消分享

#### 场景描述
用户A决定不再分享"项目讨论"到"工作协作组"

#### 前端操作
1. 编辑日程
2. 取消勾选"工作协作组"
3. 保存

#### 后端处理
```python
# 旧值: shared_to_groups: ["share_group_work", "share_group_team"]
# 新值: shared_to_groups: ["share_group_team"]

# 差异计算
old_groups = {"share_group_work", "share_group_team"}
new_groups = {"share_group_team"}

removed_groups = old_groups - new_groups  # {"share_group_work"}
added_groups = new_groups - old_groups    # {}

# 触发同步
sync_group_calendar_data(["share_group_work", "share_group_team"])
# - share_group_work: 重新汇总，该日程被移除
# - share_group_team: 重新汇总，该日程仍存在
```

---

### 2.5 流程5：前端版本检测与自动刷新

#### 轮询机制
```javascript
// 前端每30秒检测一次版本
let groupVersions = {}; // {share_group_id: version}

setInterval(() => {
    const activeTab = $('.calendar-tab.active');
    if (activeTab.data('type') === 'share-group') {
        const groupId = activeTab.data('id');
        checkGroupUpdate(groupId);
    }
}, 30000);

function checkGroupUpdate(groupId) {
    const localVersion = groupVersions[groupId] || 0;
    
    fetch(`/api/share-groups/${groupId}/check-update/?version=${localVersion}`)
        .then(res => res.json())
        .then(data => {
            if (data.has_update) {
                // 显示更新徽章
                showUpdateBadge(groupId);
                
                // 如果正在查看该群组，自动刷新
                if (isViewingGroup(groupId)) {
                    loadGroupEvents(groupId);
                }
            }
        });
}
```

#### API 设计
```python
# GET /api/share-groups/{share_group_id}/check-update/?version={local_version}

# 响应
{
    "has_update": true/false,
    "current_version": 125
}
```

---

## 🛠️ Part 3: 代码实现要点

### 3.1 models.py 修改

#### 新增三个模型类
在 `core/models.py` 文件末尾添加：

```python
class CollaborativeCalendarGroup(models.Model):
    """协作日历群组"""
    # ... (参考 Part 1 的完整定义)

class GroupMembership(models.Model):
    """群组成员关系"""
    # ... (参考 Part 1 的完整定义)

class GroupCalendarData(models.Model):
    """群组日历数据"""
    # ... (参考 Part 1 的完整定义)
```

#### 扩展 DATA_SCHEMA
在 `DATA_SCHEMA["events"]["items"]` 中添加：

```python
"shared_to_groups": {
    "type": list,
    "nullable": False,
    "default": [],
},
```

#### 数据库迁移
```bash
python manage.py makemigrations
python manage.py migrate
```

---

### 3.2 views_share_groups.py 新建文件

创建 `core/views_share_groups.py`，包含以下核心函数：

#### 函数清单
1. `create_share_group(request)` - 创建群组
2. `get_my_share_groups(request)` - 获取我的群组列表
3. `join_share_group(request)` - 加入群组（通过邀请码）
4. `leave_share_group(request, share_group_id)` - 退出群组
5. `delete_share_group(request, share_group_id)` - 删除群组（仅群主）
6. `get_share_group_events(request, share_group_id)` - 获取群组日程
7. `check_group_update(request, share_group_id)` - 检查更新
8. `sync_group_calendar_data(share_group_ids, trigger_user)` - **核心同步函数**

#### 核心函数伪代码
```python
# 1. 创建群组
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def create_share_group(request):
    # - 生成 share_group_id
    # - 创建 CollaborativeCalendarGroup
    # - 添加创建者为 owner 成员
    # - 初始化 GroupCalendarData
    # - 返回群组信息

# 2. 同步群组数据（最核心）
def sync_group_calendar_data(share_group_ids: list, trigger_user=None):
    for group_id in share_group_ids:
        # - 查询群组所有成员
        # - 遍历成员，获取其 events
        # - 筛选 shared_to_groups 包含该群组的日程
        # - 添加 owner_id, owner_name, is_readonly
        # - 保存到 GroupCalendarData.events_data
        # - 递增 version
```

---

### 3.3 views_events.py 修改

#### 修改点1：bulk_edit_events_impl 末尾（编辑弹窗触发）
在保存 `user_events_data.set_value(updated_events)` 之后添加：

```python
# 新增：同步群组数据
affected_groups = set()
for event in updated_events:
    old_groups = set(event.get('_old_shared_to_groups', []))  # 需要记录旧值
    new_groups = set(event.get('shared_to_groups', []))
    affected_groups.update(old_groups | new_groups)

if affected_groups:
    from .views_share_groups import sync_group_calendar_data
    sync_group_calendar_data(list(affected_groups), request.user)
```

#### 修改点2：update_events_impl 末尾（拖拽/快速编辑触发）
**注意**: `update_events_impl` 用于拖拽事件改变时间，也支持其他字段更新。

在保存 `user_events_data.set_value(events)` 之后添加：

```python
# 新增：同步群组数据（与 bulk_edit_events_impl 相同逻辑）
affected_groups = set()

# 方案1：如果只编辑了单个事件
if target_event:
    old_groups = set(target_event.get('_old_shared_to_groups', []))
    new_groups = set(target_event.get('shared_to_groups', []))
    affected_groups.update(old_groups | new_groups)

# 方案2：如果是重复事件且编辑了整个系列
if is_recurring and rrule_change_scope in ['all', 'future', 'from_time']:
    for event in events:
        if event.get('series_id') == series_id:
            old_groups = set(event.get('_old_shared_to_groups', []))
            new_groups = set(event.get('shared_to_groups', []))
            affected_groups.update(old_groups | new_groups)

if affected_groups:
    from .views_share_groups import sync_group_calendar_data
    sync_group_calendar_data(list(affected_groups), request.user)
```

#### 修改点3：记录旧值（两个函数都需要）
在 `bulk_edit_events_impl` 和 `update_events_impl` 修改事件之前，记录 `_old_shared_to_groups`：

```python
# 在修改事件前
for event in events:
    if event.get('id') == event_id:
        event['_old_shared_to_groups'] = event.get('shared_to_groups', []).copy()
        # ... 然后进行修改
```

#### 修改点4：create_event_impl（创建事件）
创建事件时，如果有 `shared_to_groups`，也需要触发同步：

```python
# 创建事件后
if new_event.get('shared_to_groups'):
    from .views_share_groups import sync_group_calendar_data
    sync_group_calendar_data(new_event['shared_to_groups'], request.user)
```

---

#### 📋 总结：需要修改的接口

| 接口函数 | 触发场景 | 是否需要同步 | 修改位置 |
|---------|---------|-------------|---------|
| `bulk_edit_events_impl` | 编辑弹窗保存/批量删除 | ✅ 是 | 函数末尾 |
| `update_events_impl` | 拖拽事件/快速编辑 | ✅ 是 | 函数末尾 |
| `create_event_impl` | 创建新日程 | ✅ 是 | 函数末尾 |
| `delete_event` (已弃用) | - | ❌ 否 | - |

---

### 3.4 urls.py 路由配置

在 `core/urls.py` 中添加：

```python
from . import views_share_groups

urlpatterns = [
    # ... 现有路由
    
    # 分享群组管理
    path('api/share-groups/create/', views_share_groups.create_share_group, name='create_share_group'),
    path('api/share-groups/my-groups/', views_share_groups.get_my_share_groups, name='get_my_share_groups'),
    path('api/share-groups/join/', views_share_groups.join_share_group, name='join_share_group'),
    path('api/share-groups/<str:share_group_id>/leave/', views_share_groups.leave_share_group, name='leave_share_group'),
    path('api/share-groups/<str:share_group_id>/delete/', views_share_groups.delete_share_group, name='delete_share_group'),
    
    # 群组日程
    path('api/share-groups/<str:share_group_id>/events/', views_share_groups.get_share_group_events, name='get_share_group_events'),
    path('api/share-groups/<str:share_group_id>/check-update/', views_share_groups.check_group_update, name='check_group_update'),
]
```

---

### 3.5 admin.py 注册模型

在 `core/admin.py` 中添加：

```python
from .models import CollaborativeCalendarGroup, GroupMembership, GroupCalendarData

@admin.register(CollaborativeCalendarGroup)
class CollaborativeCalendarGroupAdmin(admin.ModelAdmin):
    list_display = ('share_group_id', 'share_group_name', 'owner', 'created_at')
    search_fields = ('share_group_name', 'owner__username')

@admin.register(GroupMembership)
class GroupMembershipAdmin(admin.ModelAdmin):
    list_display = ('share_group', 'user', 'role', 'joined_at')
    list_filter = ('role',)

@admin.register(GroupCalendarData)
class GroupCalendarDataAdmin(admin.ModelAdmin):
    list_display = ('share_group', 'version', 'last_updated')
```

---

## 🎨 Part 4: 前端实现要点

### 4.1 选项卡切换 UI

#### HTML 结构（在日历卡片顶部）
```html
<!-- 在 home_new.html 中，日历卡片内部添加 -->
<div class="calendar-tabs-container">
    <!-- 我的日程 -->
    <div class="calendar-tab active" data-type="my" data-id="my-calendar">
        <i class="fas fa-user"></i>
        <span>我的日程</span>
    </div>
    
    <!-- 群组选项卡（动态渲染） -->
    <div class="calendar-tab" data-type="share-group" data-id="share_group_xxx">
        <i class="fas fa-users"></i>
        <span>工作协作</span>
        <span class="update-badge" style="display:none;">•</span>
    </div>
    
    <!-- 添加按钮 -->
    <div class="calendar-tab-add" title="创建/加入群组">
        <i class="fas fa-plus"></i>
    </div>
</div>
```

#### CSS 样式
```css
.calendar-tabs-container {
    display: flex;
    gap: 8px;
    padding: 10px;
    border-bottom: 1px solid #ddd;
    overflow-x: auto;
}

.calendar-tab {
    padding: 8px 16px;
    border-radius: 6px;
    cursor: pointer;
    transition: all 0.3s;
    display: flex;
    align-items: center;
    gap: 6px;
}

.calendar-tab:hover {
    background: #f0f0f0;
}

.calendar-tab.active {
    background: #007bff;
    color: white;
}

.update-badge {
    color: #ff4444;
    font-size: 20px;
    animation: pulse 1s infinite;
}
```

#### JavaScript 切换逻辑
```javascript
// 切换选项卡
$('.calendar-tab').on('click', function() {
    const type = $(this).data('type');
    const id = $(this).data('id');
    
    // 更新激活状态
    $('.calendar-tab').removeClass('active');
    $(this).addClass('active');
    
    // 隐藏更新徽章
    $(this).find('.update-badge').hide();
    
    if (type === 'my') {
        // 加载我的日程（现有逻辑）
        loadMyCalendar();
    } else if (type === 'share-group') {
        // 加载群组日程
        loadGroupCalendar(id);
    }
});

function loadGroupCalendar(groupId) {
    const localVersion = groupVersions[groupId] || 0;
    
    fetch(`/api/share-groups/${groupId}/events/?version=${localVersion}`)
        .then(res => res.json())
        .then(data => {
            if (data.status === 'updated') {
                // 更新本地版本号
                groupVersions[groupId] = data.version;
                
                // 渲染日程（只读模式）
                calendar.removeAllEvents();
                calendar.addEventSource(data.events);
                calendar.refetchEvents();
            }
        });
}
```

---

### 4.2 编辑弹窗添加分享选项

#### HTML（在编辑弹窗中添加）
```html
<!-- 在 editEventModal 中添加 -->
<div class="form-group">
    <label>
        <i class="fas fa-share-alt"></i>
        分享到群组
    </label>
    <div class="share-groups-selector" id="shareGroupsSelector">
        <!-- 动态渲染群组列表 -->
        <label>
            <input type="checkbox" name="share_group" value="share_group_work">
            <i class="fas fa-users"></i>
            工作协作组
        </label>
        <label>
            <input type="checkbox" name="share_group" value="share_group_family">
            <i class="fas fa-home"></i>
            家庭日历
        </label>
    </div>
</div>
```

#### JavaScript（加载群组列表）
```javascript
// 打开编辑弹窗时，加载用户的群组列表
function openEditModal(event) {
    // ... 现有逻辑
    
    // 加载群组列表
    loadShareGroupsSelector(event);
}

function loadShareGroupsSelector(event) {
    fetch('/api/share-groups/my-groups/')
        .then(res => res.json())
        .then(data => {
            const selector = $('#shareGroupsSelector');
            selector.empty();
            
            data.groups.forEach(group => {
                const isChecked = event.shared_to_groups?.includes(group.share_group_id);
                selector.append(`
                    <label>
                        <input type="checkbox" name="share_group" 
                               value="${group.share_group_id}"
                               ${isChecked ? 'checked' : ''}>
                        <i class="fas fa-users" style="color:${group.share_group_color}"></i>
                        ${group.share_group_name}
                    </label>
                `);
            });
        });
}

// 保存时，收集选中的群组
function saveEvent() {
    const selectedGroups = [];
    $('input[name="share_group"]:checked').each(function() {
        selectedGroups.push($(this).val());
    });
    
    const eventData = {
        // ... 其他字段
        shared_to_groups: selectedGroups
    };
    
    // 发送保存请求（使用现有 API）
    updateEvent(eventData);
}
```

---

### 4.3 只读事件渲染

#### 事件渲染函数修改
```javascript
// FullCalendar 事件渲染钩子
eventDidMount: function(info) {
    const event = info.event;
    
    // 判断是否为只读事件（来自群组）
    if (event.extendedProps.is_readonly) {
        // 添加只读样式
        info.el.classList.add('readonly-event');
        
        // 添加锁图标
        const lockIcon = document.createElement('i');
        lockIcon.className = 'fas fa-lock readonly-icon';
        info.el.querySelector('.fc-event-title').prepend(lockIcon);
        
        // 添加 owner 信息
        const ownerBadge = document.createElement('span');
        ownerBadge.className = 'owner-badge';
        ownerBadge.textContent = event.extendedProps.owner_name;
        info.el.querySelector('.fc-event-title').append(ownerBadge);
    }
    
    // 判断是否为已分享事件（自己的日程）
    if (event.extendedProps.shared_to_groups?.length > 0) {
        // 添加分享标识
        info.el.classList.add('shared-event');
        
        const shareIcon = document.createElement('i');
        shareIcon.className = 'fas fa-share-alt share-icon';
        shareIcon.title = '已分享到 ' + event.extendedProps.shared_to_groups.length + ' 个群组';
        info.el.querySelector('.fc-event-title').prepend(shareIcon);
    }
}
```

#### CSS 样式
```css
/* 只读事件样式 */
.readonly-event {
    background: #f5f5f5 !important;
    border-left: 4px solid #999 !important;
    opacity: 0.8;
    cursor: not-allowed;
}

.readonly-icon {
    margin-right: 4px;
    color: #666;
}

.owner-badge {
    margin-left: 6px;
    padding: 2px 6px;
    background: #666;
    color: white;
    border-radius: 3px;
    font-size: 11px;
}

/* 已分享事件样式 */
.shared-event {
    border-left: 4px solid #007bff !important;
}

.share-icon {
    margin-right: 4px;
    color: #007bff;
}
```

#### 点击事件处理
```javascript
eventClick: function(info) {
    const event = info.event;
    
    if (event.extendedProps.is_readonly) {
        // 只读事件，显示提示
        alert(`这是来自 ${event.extendedProps.owner_name} 的共享日程，仅可查看`);
        
        // 显示详情弹窗（只读模式）
        showReadonlyEventDetails(event);
    } else {
        // 可编辑事件，打开编辑弹窗
        openEditModal(event);
    }
}
```

---

### 4.4 群组管理界面

#### 创建群组弹窗
```html
<!-- 点击"+"按钮时打开 -->
<div id="createGroupModal" class="modal">
    <div class="modal-content">
        <h3>创建协作群组</h3>
        <form id="createGroupForm">
            <div class="form-group">
                <label>群组名称</label>
                <input type="text" name="share_group_name" required>
            </div>
            <div class="form-group">
                <label>颜色标签</label>
                <input type="color" name="share_group_color" value="#3498db">
            </div>
            <div class="form-group">
                <label>描述</label>
                <textarea name="share_group_description"></textarea>
            </div>
            <button type="submit">创建</button>
        </form>
    </div>
</div>
```

#### JavaScript
```javascript
$('#createGroupForm').on('submit', function(e) {
    e.preventDefault();
    
    const formData = {
        share_group_name: $('input[name="share_group_name"]').val(),
        share_group_color: $('input[name="share_group_color"]').val(),
        share_group_description: $('textarea[name="share_group_description"]').val()
    };
    
    fetch('/api/share-groups/create/', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(formData)
    })
    .then(res => res.json())
    .then(data => {
        if (data.status === 'success') {
            // 刷新群组列表
            refreshShareGroupTabs();
            $('#createGroupModal').hide();
        }
    });
});
```

---

## 📋 Part 5: 实施步骤

### Phase 1: 数据库准备（预计1小时）

#### Step 1.1: 修改 models.py
- [ ] 在 `core/models.py` 末尾添加三个新模型
  - CollaborativeCalendarGroup
  - GroupMembership
  - GroupCalendarData
- [ ] 在 `DATA_SCHEMA["events"]["items"]` 中添加 `shared_to_groups` 字段

#### Step 1.2: 数据库迁移
```bash
python manage.py makemigrations core
python manage.py migrate
```

#### Step 1.3: 注册到 admin
- [ ] 修改 `core/admin.py`，注册三个新模型
- [ ] 测试：访问 `/admin/`，确认可以看到新模型

---

### Phase 2: 后端核心功能（预计4-6小时）

#### Step 2.1: 创建 views_share_groups.py
- [ ] 新建文件 `core/views_share_groups.py`
- [ ] 实现 8 个核心函数：
  1. `create_share_group` - 创建群组
  2. `get_my_share_groups` - 获取我的群组
  3. `join_share_group` - 加入群组
  4. `leave_share_group` - 退出群组
  5. `delete_share_group` - 删除群组
  6. `get_share_group_events` - 获取群组日程
  7. `check_group_update` - 检查更新
  8. **`sync_group_calendar_data`** - 同步核心函数

#### Step 2.2: 修改 views_events.py
- [ ] 在 **`bulk_edit_events_impl`** 末尾添加同步逻辑（编辑弹窗触发）
- [ ] 在 **`update_events_impl`** 末尾添加同步逻辑（拖拽/快速编辑触发）
- [ ] 在两个函数中，更新事件前记录 `_old_shared_to_groups`
- [ ] 在 **`create_event_impl`** 中添加同步逻辑（创建日程触发）

#### Step 2.3: 配置路由
- [ ] 修改 `core/urls.py`，添加 7 条新路由

#### Step 2.4: 测试后端 API
使用 Postman 或 curl 测试：
```bash
# 创建群组
POST /api/share-groups/create/
Body: {"share_group_name": "测试群组", "share_group_color": "#ff0000"}

# 获取我的群组
GET /api/share-groups/my-groups/

# 分享日程（修改现有日程，添加 shared_to_groups）
PUT /api/events/bulk-edit/
Body: {"event_id": "xxx", "shared_to_groups": ["share_group_xxx"]}

# 获取群组日程
GET /api/share-groups/share_group_xxx/events/?version=0
```

---

### Phase 3: 前端 UI 实现（预计6-8小时）

#### Step 3.1: 选项卡切换组件
- [ ] 修改 `templates/home_new.html`
  - 在日历卡片顶部添加 `.calendar-tabs-container`
  - 移除"日程日历"标题
- [ ] 添加 CSS 样式（可在 `static/css/` 中新建 `share-groups.css`）
- [ ] 实现 JavaScript 逻辑：
  - 切换选项卡
  - 加载群组日程
  - 版本检测轮询（30秒）

#### Step 3.2: 编辑弹窗修改
- [ ] 在编辑日程弹窗中添加"分享到群组"选项
- [ ] 实现群组列表动态加载
- [ ] 保存时收集 `shared_to_groups` 数据

#### Step 3.3: 事件渲染样式
- [ ] 添加只读事件样式（灰色背景+锁图标）
- [ ] 添加已分享事件样式（蓝色边框+分享图标）
- [ ] 实现点击只读事件时的提示

#### Step 3.4: 群组管理界面
- [ ] 创建群组弹窗
- [ ] 加入群组弹窗（通过邀请码）
- [ ] 群组成员管理界面

#### Step 3.5: 更新提示
- [ ] 实现版本检测轮询
- [ ] 显示更新徽章
- [ ] 自动刷新逻辑

---

### Phase 4: 集成测试（预计2-3小时）

#### Test Case 1: 创建群组并分享日程
1. 用户A创建群组"工作协作"
2. 用户A编辑日程"项目讨论"，分享到"工作协作"
3. 检查 GroupCalendarData 中是否有该日程
4. 用户B加入"工作协作"
5. 用户B切换到"工作协作"选项卡，确认可以看到日程

#### Test Case 2: 编辑已分享的日程
1. 用户A修改"项目讨论"的标题和时间
2. 检查 GroupCalendarData 是否更新
3. 用户B刷新，确认看到最新内容

#### Test Case 3: 删除已分享的日程
1. 用户A删除"项目讨论"
2. 检查 GroupCalendarData 中是否移除该日程
3. 用户B刷新，确认日程消失

#### Test Case 4: 取消分享
1. 用户A取消"项目讨论"的分享
2. 检查 GroupCalendarData 中是否移除该日程
3. 用户B刷新，确认日程消失

#### Test Case 5: RRule 重复日程
1. 用户A创建重复日程"每周例会"（FREQ=WEEKLY）
2. 选择"编辑全部"，分享到群组
3. 检查所有实例是否都有 `shared_to_groups`
4. 用户B确认可以看到所有重复实例

#### Test Case 6: 版本检测
1. 用户B打开"工作协作"选项卡
2. 用户A分享新日程
3. 等待30秒（轮询间隔）
4. 确认用户B看到更新徽章
5. 点击选项卡，确认自动刷新

---

### Phase 5: 优化与边界条件（预计2-3小时）

#### 边界条件处理
- [ ] 用户删除自己的群组时，清理 GroupCalendarData
- [ ] 用户退出群组时，重新同步群组数据
- [ ] 群组被删除时，清理所有成员的 `shared_to_groups`
- [ ] 同步时，过滤掉已被删除的事件
- [ ] 处理网络请求失败的情况

#### 性能优化
- [ ] 同步任务异步化（可选，使用 Celery）
- [ ] 群组日程数据缓存（前端）
- [ ] 减少不必要的同步触发

#### 安全性
- [ ] 验证用户是否是群组成员
- [ ] 验证群主权限
- [ ] 防止 SQL 注入（Django ORM 已处理）
- [ ] API 接口限流（可选）

---

## 🚨 已知问题与待解决

### 问题1: 大规模群组性能
**现象**: 群组成员很多时，同步可能很慢  
**解决方案**:
- 使用 Celery 异步任务
- 增量同步（只同步变化的事件）
- 数据库索引优化

### 问题2: RRule 实例数量庞大
**现象**: 无限重复的日程可能生成大量实例  
**解决方案**:
- 限制生成实例的时间范围（如未来1年）
- 按需生成（lazy loading）

### 问题3: 网络延迟
**现象**: 用户B可能不会立即看到更新  
**解决方案**:
- 缩短轮询间隔（如15秒）
- 未来可升级为 WebSocket

---

## 📚 附录

### A. API 接口文档

#### 1. 创建群组
```
POST /api/share-groups/create/
Headers: Authorization: Token xxx

Request:
{
    "share_group_name": "工作协作组",
    "share_group_color": "#3498db",
    "share_group_description": "项目协作用"
}

Response:
{
    "status": "success",
    "group": {
        "share_group_id": "share_group_xxx",
        "share_group_name": "工作协作组",
        "share_group_color": "#3498db",
        "owner_id": 1,
        "created_at": "2025-11-11T10:00:00"
    }
}
```

#### 2. 获取我的群组
```
GET /api/share-groups/my-groups/
Headers: Authorization: Token xxx

Response:
{
    "groups": [
        {
            "share_group_id": "share_group_work",
            "share_group_name": "工作协作组",
            "share_group_color": "#3498db",
            "role": "owner",
            "member_count": 5
        }
    ]
}
```

#### 3. 获取群组日程
```
GET /api/share-groups/{share_group_id}/events/?version=124
Headers: Authorization: Token xxx

Response (有更新):
{
    "status": "updated",
    "version": 125,
    "events": [
        {
            "id": "event_123",
            "title": "项目讨论",
            "start": "2025-11-12T10:00:00",
            "end": "2025-11-12T11:00:00",
            "owner_id": 1,
            "owner_name": "张三",
            "is_readonly": true,
            "shared_at": "2025-11-11T09:00:00",
            ...
        }
    ],
    "last_updated": "2025-11-11T10:30:00"
}

Response (无更新):
{
    "status": "no_update",
    "version": 124
}
```

#### 4. 检查更新
```
GET /api/share-groups/{share_group_id}/check-update/?version=124
Headers: Authorization: Token xxx

Response:
{
    "has_update": true,
    "current_version": 125
}
```

---

### B. 数据库表结构

#### collaborative_calendar_group
| 字段 | 类型 | 说明 |
|------|------|------|
| share_group_id | VARCHAR(100) PK | 群组ID |
| share_group_name | VARCHAR(200) | 群组名称 |
| share_group_color | VARCHAR(20) | 颜色标签 |
| share_group_description | TEXT | 描述 |
| owner_id | INT FK | 群主ID |
| created_at | DATETIME | 创建时间 |
| updated_at | DATETIME | 更新时间 |

#### group_membership
| 字段 | 类型 | 说明 |
|------|------|------|
| id | INT PK AUTO | 主键 |
| share_group_id | VARCHAR(100) FK | 群组ID |
| user_id | INT FK | 用户ID |
| role | VARCHAR(20) | 角色 |
| joined_at | DATETIME | 加入时间 |
| UNIQUE(share_group_id, user_id) |

#### group_calendar_data
| 字段 | 类型 | 说明 |
|------|------|------|
| id | INT PK AUTO | 主键 |
| share_group_id | VARCHAR(100) FK UNIQUE | 群组ID |
| events_data | JSON | 事件数据 |
| version | INT | 版本号 |
| last_updated | DATETIME | 最后更新时间 |

---

### C. 前端状态管理

#### 全局变量
```javascript
// 群组版本号缓存
let groupVersions = {
    'share_group_work': 124,
    'share_group_family': 56
};

// 当前激活的日历
let activeCalendar = {
    type: 'my',  // 'my' 或 'share-group'
    id: 'my-calendar'  // 或 'share_group_xxx'
};

// 轮询定时器
let updateCheckInterval = null;
```

#### 关键函数
```javascript
// 初始化
function initShareGroups() {
    loadShareGroupTabs();
    startUpdatePolling();
}

// 加载群组选项卡
function loadShareGroupTabs() {
    fetch('/api/share-groups/my-groups/')
        .then(res => res.json())
        .then(data => {
            renderGroupTabs(data.groups);
        });
}

// 开始轮询
function startUpdatePolling() {
    updateCheckInterval = setInterval(() => {
        if (activeCalendar.type === 'share-group') {
            checkGroupUpdate(activeCalendar.id);
        }
    }, 30000);
}

// 停止轮询
function stopUpdatePolling() {
    if (updateCheckInterval) {
        clearInterval(updateCheckInterval);
    }
}
```

---

## ✅ 验收标准

### 功能验收
- [ ] 用户可以创建群组并邀请成员
- [ ] 用户可以将日程分享到群组
- [ ] 群组成员可以查看共享日程（只读）
- [ ] 分享者编辑/删除日程后，群组内实时同步
- [ ] RRule 重复日程可以整体分享
- [ ] 前端版本检测正常，有更新时提示

### 性能验收
- [ ] 单次同步耗时 < 3秒（100个成员，1000个事件）
- [ ] 前端轮询不影响用户操作
- [ ] 页面加载时间 < 2秒

### 安全验收
- [ ] 非群组成员无法访问群组日程
- [ ] 只有群主可以删除群组
- [ ] API 接口有身份验证

### UI/UX 验收
- [ ] 选项卡切换流畅
- [ ] 只读事件样式明显区分
- [ ] 更新提示清晰
- [ ] 移动端适配良好

---

**文档完成！** 🎉



