...
P4 的详细执行顺序、统一应用服务、Agent/Quick Action/MCP 适配、内部附件和会话回滚窗口方案见 [P4 Agent、Quick Action、MCP、附件与会话回滚升级详细方案](./P4-Agent-QuickAction-MCP-附件与会话回滚升级详细方案.md)。

关键决策：所有协议入口共同调用 `PlannerApplicationService`，不互相调用 View/API，也不自行复制 ORM/RRule 写逻辑；回滚继续采用操作前快照恢复，但快照缩小为当前命令受影响 aggregate 的临时压缩 payload，并在会话切换关闭 rollback window 后删除。P4 不迁移 cutover 前旧回滚历史，旧请求直接返回 410。生产清理完成后使用安全数据库压缩流程实际释放 SQLite 空间。

## 9. P4 完成结论（2026-07-12）
