...
urlpatterns = [
    ...
    path('api/events/groups/', views.get_events_groups, name='get_events_groups'),
    ...
    path('api/reminders/convert-to-single/', views.convert_recurring_to_single, name='convert_recurring_to_single'),  # bulk-edit 若检测到用户把原先有 RRule 参数的日程的 RRule 去除掉了，就会给前端一个报错，前端再检查，会发现这是一个将重复日程改为但此日程的特殊操作，就会进一步在前端链接这个函数
